...
//	PulseGenerator gatePulse;
	...
	void reset() override {
		gateBits = LFSR_MASK;		
		tapBits = HIGH_BIT;
		lfsrBits = HIGH_BIT;		/* double whatevers */
	}
...
void FG8::step() {
	...
	bool galoisType = params[MODE_PARAM].value > 0.0f;
...
//...	outputs[GATES_OUTPUT].value = gatesOn ? 10.0 : 0.0;
	outputs[GATES_OUTPUT].value = (gateIn && ((lfsrBits & LFSR_MASK) & (gateBits & LFSR_MASK))) ? 10.0f : 0.0f;
	lights[RESET_LIGHT].value = resetLight;
//...	lights[GATES_LIGHT].value = gatesOn ? 1.0 : 0.0;
	lights[GATES_LIGHT].value = (gateIn && ((lfsrBits & LFSR_MASK) & (gateBits & LFSR_MASK))) ? 0.85 : 0.0;
...
FG8Widget::FG8Widget() {
	...
	addParam(createParam<RoundSmallBlackKnob>(Vec(18, 56), module, FG8::CLOCK_PARAM, -2.0, 6.0, 2.0));
	addParam(createParam<LEDButton>(Vec(60, 61-1), module, FG8::RUN_PARAM, 0.0, 1.0, 0.0));
	addChild(createLight<MediumLight<GreenLight>>(Vec(64.4, 64.4), module, FG8::RUNNING_LIGHT));
	addParam(createParam<LEDButton>(Vec(99, 61-1), module, FG8::RESET_PARAM, 0.0, 1.0, 0.0));
...
//	addParam(createParam<RoundSmallBlackSnapKnob>(Vec(132, 56), module, FG8::STEPS_PARAM, 1.0, 8.0, 8.0));
	...
	addParam(createParam<CKSS>(Vec(139, 74), module, FG8::MODE_PARAM, 0.0, 1.0, 1.0));
//	addParam(createParam<NKK>(Vec(139, 74), module, FG8::MODE_PARAM, 0.0, 1.0, 1.0));

	addChild(createLight<MediumLight<GreenLight>>(Vec(179.4, 64.4), module, FG8::GATES_LIGHT));
	addChild(createLight<MediumLight<GreenLight>>(Vec(218.4, 64.4), module, FG8::ROW_LIGHTS));
	addChild(createLight<MediumLight<GreenLight>>(Vec(256.4, 64.4), module, FG8::ROW_LIGHTS + 1));
	addChild(createLight<MediumLight<GreenLight>>(Vec(295.4, 64.4), module, FG8::ROW_LIGHTS + 2));
...
//	addInput(createInput<PJ301MPort>(Vec(portX0[3]-1, line98), module, FG8::STEPS_INPUT));
	...
	addOutput(createOutput<PJ301MPort>(Vec(portX0[4]-1, line98), module, FG8::GATES_OUTPUT));
	addOutput(createOutput<PJ301MPort>(Vec(portX0[5]-1, line98), module, FG8::ROW1_OUTPUT));
	addOutput(createOutput<PJ301MPort>(Vec(portX0[6]-1, line98), module, FG8::ROW2_OUTPUT));
	addOutput(createOutput<PJ301MPort>(Vec(portX0[7]-1, line98), module, FG8::ROW3_OUTPUT));
...
/* change row knobs to snap octave 12 step knobs */
		addParam(createParam<RoundSmallBlackSnapKnob>(Vec(portX1[i]-2, 157), module, FG8::ROW1_PARAM + i, 0.0, 12.0, 0.0));
		addParam(createParam<RoundSmallBlackSnapKnob>(Vec(portX1[i]-2, 198), module, FG8::ROW2_PARAM + i, 0.0, 12.0, 0.0));
		addParam(createParam<RoundSmallBlackSnapKnob>(Vec(portX1[i]-2, 240), module, FG8::ROW3_PARAM + i, 0.0, 12.0, 0.0));
		
		addParam(createParam<LEDButton>(Vec(portX1[i]+2, 278-1), module, FG8::GATE_PARAM + i, 0.0, 1.0, 0.0));
		addChild(createLight<MediumLight<GreenLight>>(Vec(portX1[i]+6.4, 281.4), module, FG8::GATE_LIGHTS + i));
//		addOutput(createOutput<PJ301MPort>(Vec(portX[i]-1, 307), module, FG8::GATE_OUTPUT + i));

		addParam(createParam<LEDBezel>(Vec(portX1[i], 307), module, FG8::TAP_PARAM + i, 0.0, 1.0, 0.0));
		addChild(createLight<TapLight<BlueLight>>(Vec(portX1[i] + 2, 307 + 2), module, FG8::TAP_LIGHTS + i));

		addParam(createParam<LEDButton>(Vec(portX1[i]+2, 123), module, FG8::LFSR_PARAM + i, 0.0, 1.0, 0.0));
		addChild(createLight<MediumLight<RedLight>>(Vec(portX1[i]+6.4, 127), module, FG8::LFSR_LIGHTS + i));
	}
}

/* no menu items, no menu.
struct FG8GateModeItem : MenuItem {
Menu *FG8Widget::createContextMenu() {
*/
