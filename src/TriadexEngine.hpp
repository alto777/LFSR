...
class TriadexEngine {
  public:
	...
	int bitValue(int);						/* need for lamp control - add setBitValue one day? */
...
int TriadexEngine::bitValue(int slot)
{
...
float TriadexEngine::getNote()
{
	...
	return ((scale[note & 0x7] + ((note & 0x8) ? 12 : 0)) / 12.0);
}

int TriadexEngine::claculateLFSRBit()
{
	bool bit = 1;		/* XNOR, right? :-) */
